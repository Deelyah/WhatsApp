  ...
  <div>
    <div class="flex items-center relative">
      ...
      <div class="flex ml-auto items-center">
        ...
        >
          <img src="~/assets/kebab-menu.png" alt="icon" title="Menu" />
        </button>
      </div>
      <Dropdown
...
<!-- color: var(--panel-header-icon); font: inherit; font-size: 100%; vertical-align:
initial; outline: none; margin: 0; border: 0; position: absolute;  overflow: visible; top: 44px; padding: 9px 0; background-color:
var(--dropdown-background); border-radius: 3px; box-shadow: 0 2px 5px 0
rgba(var(--shadow-rgb),.26),0 2px 10px 0 rgba(var(--shadow-rgb),.16);
text-align: left; right: 4px; transform-origin: right top; transform: scale(1);
opacity: 1; -->
...
export default {
  ...
  methods: {
    toggleNewChat() {
      this.$store.commit("toggleNewChat");
      alert(this.$store.getters.displayChat);
    },
  },
  components: { Dropdown },
};
